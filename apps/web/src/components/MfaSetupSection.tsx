  ...
  return (
    <div className="bg-white rounded-lg border border-[#F3E8D8] p-6">
      <div className="flex items-center gap-3 mb-1">
        ...
        <h2 className="text-lg font-semibold text-amber-900">{t("mfa.title")}</h2>
      </div>
      <p className="text-sm text-amber-700 mb-4">{t("mfa.description")}</p>
          ...
          {mfaEnabled ? (
            <div className="space-y-3">
              <div className="flex items-center gap-2">
                ...
                {mfaVerifiedAt && (
                  <span className="text-xs text-amber-600" suppressHydrationWarning>
            ...
            <div className="space-y-3">
              ...
              <div>
                <button
                  ...
                  disabled={loading}
                  className="px-4 py-2 bg-gradient-to-r from-amber-500 to-amber-600 text-white rounded-lg hover:from-amber-600 hover:to-amber-700 transition-colors disabled:opacity-50 text-sm"
                >
      ...
      {step === "setup" && (
        <div className="space-y-4">
          <div className="flex flex-col items-center gap-3">
            <p className="text-sm font-medium text-amber-800">{t("mfa.scanQr")}</p>
            {qrUrl && (
              <img src={qrUrl} alt="QR Code" width={200} height={200} className="rounded-lg border border-[#F3E8D8]" />
            )}
          </div>
          <div>
            <p className="text-xs text-amber-600 mb-1">{t("mfa.orEnterManually")}</p>
            <code className="block px-3 py-2 bg-[#FFFBF5] rounded border border-[#F3E8D8] text-sm font-mono break-all select-all">
              {secretKey}
            </code>
          </div>
          <form onSubmit={handleVerify} className="space-y-3">
            ...
            <button
              ...
              className="w-full px-4 py-2 bg-gradient-to-r from-amber-500 to-amber-600 text-white rounded-lg hover:from-amber-600 hover:to-amber-700 transition-colors disabled:opacity-50"
            >
      ...
      {step === "backup" && (
        <div className="space-y-4">
          ...
          <div>
            <h3 className="text-sm font-semibold text-amber-900 mb-1">{t("mfa.backupCodes")}</h3>
            <p className="text-xs text-amber-700 mb-2">{t("mfa.backupCodesDesc")}</p>
            ...
            <div className="grid grid-cols-2 gap-2 mb-3">
              {backupCodes.map((bc, i) => (
                <code key={i} className="px-3 py-1.5 bg-[#FFFBF5] rounded border border-[#F3E8D8] text-sm font-mono text-center">
                  {bc}
                </code>
              ))}
            </div>
            <button
              ...
              className="flex items-center gap-2 px-3 py-1.5 bg-amber-50 text-amber-800 rounded-lg hover:bg-amber-100 transition-colors text-sm"
            >
            ...
            <button
              ...
              className="w-full px-4 py-2 bg-gradient-to-r from-amber-500 to-amber-600 text-white rounded-lg hover:from-amber-600 hover:to-amber-700 transition-colors"
            >
            {t("common.close")}
          </button>
        </div>
      ...
      {step === "disable" && (
        <div className="space-y-3">
          <p className="text-sm text-amber-700">{t("mfa.disableConfirm")}</p>
